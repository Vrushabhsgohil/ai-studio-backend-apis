...
class OrchestrationService(BaseService):
    ...
    def _run_fashion_visual_agent(self, concept: str) -> str:
        ...
        """
        return openai_service.chat_completion("gpt-4o-mini", [{"role": "system", "content": prompt}, {"role": "user", "content": concept}])
    ...
    def _run_fashion_audio_agent(self, concept: str, visuals: str, voice_over: bool, vibe: str) -> str:
        ...
        return openai_service.chat_completion("gpt-4o-mini", [{"role": "system", "content": prompt}, {"role": "user", "content": f"{concept}\n{visuals}"}])

    def _run_fashion_assembly_agent(self, concept: str, visuals: str, audio: str) -> str:
        prompt = "Assemble final SORA-2 prompt paragraph. PRIORITIZE USER CONTENT. Append: 'REFERENCE IMAGE RULE: The product visual attributes must match reference exactly.'"
        return openai_service.chat_completion("gpt-4o-mini", [{"role": "system", "content": prompt}, {"role": "user", "content": f"{concept}\n{visuals}\n{audio}"}])
    ...
    def _run_promo_assembly_agent(self, concept: str, visuals: str, audio: str) -> str:
        prompt = "Assemble final SORA-2 prompt paragraph for PRODUCT-ONLY commercial. ZERO HUMAN REFERENCES."
        return openai_service.chat_completion("gpt-4o-mini", [{"role": "system", "content": prompt}, {"role": "user", "content": f"{concept}\n{visuals}\n{audio}"}])

    def _run_qa_loop(self, agent_role: str, initial_prompt: str, user_content: str, v_type: str) -> (str, Dict):
        qa_prompt = f"You are a {v_type} quality controller. Score 0-100. Need >= 80 to pass. Return JSON: approved, score, violations, qa_summary."
